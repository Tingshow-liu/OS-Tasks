...
void exit_handler(Process* exitProc) {
    ...
    for (int i = 0; i < pageTableSize; i++) {
        ...
        if (exitProc->pageTable[i].PRESENT) {
            ...
            // cout << "frameToFree: " << exitProc->pageTable[i].FRAMENUMBER << endl;  // TBD
        }
    }
}
...
// Define a function to implement aging and access update
void ageFrames(vector<Frame_t>& frameTable) {
    for (Frame_t& frame: frameTable) {
        if (frame.inUse) {
            frame.age >>= 1;  // Right shift the page by 1 position
            // If the page was referenced recently, set its leading bit to 1
            if (frame.process->pageTable[frame.vPage].REFERENCED) {
                frame.age = (frame.age | 0x80000000);
                frame.process->pageTable[frame.vPage].REFERENCED = 0;  // Reset the referenced bit after use
            }
        }
    }
}
...
class Aging: public Pager {
    ...
    public:
        ...
        Frame_t* select_victim_frame() override {
            ageFrames(frameTable);  // First age all frames
            ...
            Frame_t* victimFrame = nullptr;
            for (Frame_t& currFrame: frameTable) {
...
// Define a class to update frame usage only for the working-set pager
int currentTime = 0;  // Used to calculate TAU (by instructions)
void updateTimeLastUsed(vector<Frame_t>& frameTable) {
    if (workingSet) {
        static const int TAU = 49;  // Time criteria
        for (Frame_t& currFrame: frameTable) {
            currFrame.canReplace = false;  // First resset this parameter
            // Check if the process pointer is valid before accessing its members
            if (currFrame.process && currFrame.vPage != -1) {
                // Record its previous status to print the correct summary table
                ...
                if (currFrame.process->pageTable[currFrame.vPage].REFERENCED) {
                    ...
                    if (currentTime - currFrame.time_last_used > TAU) {
    ...
    public:
        ...
        Frame_t* select_victim_frame() override {
            ...
            for (int i = 0; i < frameTable.size(); i++) {
                ...
                if (currFrame.canReplace) {  // This frame is selected as the victim frame by "updateTimeLastUsed" function
                    victimFrame = &currFrame;
                }
                // cout << "page: " << currFrame.vPage << " currFrame.time_last_used: " << currFrame.time_last_used << " currentTime: "  << currentTime << " Referenced? " << currFrame.process->pageTable[currFrame.vPage].REFERENCED << endl;  // TBD
                // If find a victimFrame that satisfies the condition, return directly
                if (victimFrame) {  
                    hand = (victimFrame->f_id + 1) % frameTable.size();  // Update hand for the next round
                    return victimFrame;
                }
                // Need to record the oldest frame in case there's no frame satisfies the TAU conidtion after the loop
                if (currFrame.time_last_used < oldestTime) {  // The smaller the time_last_used is, the older the frame is
...
Frame_t* get_frame() {
    ...
    // cout << "# of free frames: " << freeFrames.size() << endl;  // TBD
    if (!freeFrames.empty()) {
...
void pagefault_handler(Pte_t* pte, vector<Vma>* vmaTable, int vpage) {
    ...
    if (frame->inUse) {
        // cout << "Unmap called" << endl;  // TBD
        // cout << "Victim frame: " << frame->f_id << endl;  // TBD
...
void summary_printer(const Process& proc) {
    ...
    if (proc.processId == processTable.size() - 1) {
        ...
        inst_count, ctx_switches, process_exits, totalCost, pte_t_size);  // pte_t_size? 4? for the last field?
    }
}
...
int main(int argc, char *argv[]) {
    ...
    if (algo == 'w') { 
        pager = new WorkingSet(frameTable); 
        workingSet = true;
    }
    ...
    while (get_next_instruction(operation, vpage)) {
        ...
        // cout << "instrCounter: " << instrCounter << endl;  // TBD
        switch (operation) {
            case 'c':
...
// ./mmu -f16 -aw -oOPFS ./in1 ./rfile
// ./mmu -f32 -aw -oOPFS ./in7 ./rfile